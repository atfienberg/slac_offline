...
void crunchStruck(struck& s, 
  ...
  for(unsigned int j = 0; j < devices.size(); ++j){
    ...
    for( int k = 0; k < CHOPPED_TRACE_LENGTH; ++k){
      sr[j].trace[k] = 
	s.trace[devices[j].channel][sFitters[j]->getFitStart()+k];
    }
     
    //get summary information srom the trace
    sr[j].aSum = sFitters[j]->getSum(s.trace[devices[j].channel],
				    sFitters[j]->getFitStart(),
				    sFitters[j]->getFitLength());

    sr[j].aAmpl = sFitters[j]->getMax(sFitters[j]->getFitStart(), 
				     sFitters[j]->getFitLength());
      
    //do the fits
    if(sFitters[j]->isFitConfigured()){
      sFitters[j]->fitSingle(s.trace[devices[j].channel]);
    
      sr[j].energy = sFitters[j]->getScale();
      sr[j].chi2 = sFitters[j]->getChi2();
      sr[j].time = sFitters[j]->getTime();
      sr[j].valid = sFitters[j]->wasValidFit();
	
      //fill fitTrace
      sFitters[j]->fillFitTrace(sr[j].fitTrace,
			       sFitters[j]->getFitStart(),
			       CHOPPED_TRACE_LENGTH);
       
    }
    sr[j].baseline = sFitters[j]->getBaseline();      
  }
}
