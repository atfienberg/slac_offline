...
void howManyPe(){
  ...
  const char* files[numRuns] = {
    "../../crunchedFiles/run_00689_crunched.root",
    "../../crunchedFiles/run_00690_crunched.root",
    "../../crunchedFiles/run_00691_crunched.root",
    "../../crunchedFiles/run_00692_crunched.root",
    "../../crunchedFiles/run_00693_crunched.root",
    "../../crunchedFiles/run_00694_crunched.root"
				};
...
*/	
  ...
  for(int i = 0; i <24; ++i){
    cout << "SiPM " << 2+i/6 << 2+i%6 << " --- " 
	 << "p1 : " << gains[i] << ", "
	 << pes[i] << " photoelectrons at open filter, " << gains[i]*pes[i] << "calibration constant." << endl;
  }

  
  TGraphErrors* peGainGraph = new TGraphErrors(24, gains, pes, gainErrors, peErrors);
  ...
  TH2F* peMap = new TH2F("pes","PE Map", 6,0,6,4,0,4);
  for(int i = 0; i < 24; ++i){
    peMap->Fill(.5+i%6, 3.5-i/6, pes[i]);
  }
  ...
  TH2F* gainMap = new TH2F("gains","Gain Map", 6,0,6,4,0,4);
  for(int i = 0; i < 24; ++i){
    gainMap->Fill(.5+i%6, 3.5-i/6, gains[i]);
  }
