...
void howManyPe(){
  ...
  for(int row = 2; row < 6; ++row){
	...
	TF1* quad = new TF1(Form("quad%i%i",row,col),
			    "[0]+[1]*x+[2]*x*x",
			    0,100000);
	...
	quadGraph->Fit(Form("quad%i%i",row,col),"q");
	...
	*/
	...
	pes[index] = 1.0/quad->GetParameter(1)*means[0];
	gains[index] = quad->GetParameter(1);
	gainErrors[index] = quad->GetParError(1);
	peErrors[index] = TMath::Sqrt((1/gains[index]*meanErrs[0])*(1/gains[index]*meanErrs[0])+
	(means[0]/(gains[index]*gains[index])*gainErrors[index])*(means[0]/(gains[index]*gains[index])*gainErrors[index]));
